[index.html](https://github.com/user-attachments/files/25129748/index.html)
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<div class="mbg" id="modal" onclick="if(event.target===this)closeModal()">
  <div class="mdl">
    <h3>⚙️ Settings</h3>
    <p>Enter your Anthropic API key for AI Trail Advisor. Stored only in this browser. <a href="https://console.anthropic.com/settings/keys" target="_blank">Get a key →</a></p>
    <label>API Key</label>
...
<header class="hdr"><div class="hdr-in"><div><div class="hdr-left"><span style="font-size:24px">⛰️</span><h1>Trail Weather Advisor</h1></div><div class="sub">Weather-smart recommendations for serious day hikers</div></div><button class="gear" onclick="openModal()" title="Settings">⚙️</button></div></header>

<main class="mn">
  <div class="swrap" id="swrap"><div class="sbox" id="sbox"><span style="font-size:16px;opacity:.4">🔍</span><input type="text" id="sinput" placeholder="Search a location — e.g. Sedona AZ, Catskills NY..." autocomplete="off"><span id="sbusy" style="font-size:12px;color:var(--txmu);display:none">searching...</span></div><div class="gwarn" id="gwarn">Live search unavailable — showing matching presets.</div><div class="sdd" id="sdd"></div></div>
  <div class="frow" id="frow"></div>
  <div class="lh" id="lh"><div style="flex:1"><h2 id="locn"></h2><span class="rg" id="locr"></span></div><button class="svb" id="svb" onclick="toggleFav()">☆ Save</button></div>
  <div class="ldg" id="ldg"><div class="li">⛰️</div><div id="ldgt">Fetching forecast...</div></div>
  <div class="erb" id="erb"><div class="em" id="erm"></div><button onclick="doRetry()">Retry</button></div>
  <div id="empty"><div class="emp"><div class="big">🥾</div><h2>Where are you hiking?</h2><p>Search for a location or pick a popular area below for a 7-day forecast with trail scores, hourly breakdowns, and AI trail recommendations.</p></div><div id="keyBnr" style="text-align:center;margin-bottom:18px"></div><div class="pttl">Popular Hiking Areas</div><div class="pgr" id="pgr"></div></div>
  <div id="wxc" style="display:none"><div class="drow" id="drow"></div><div id="ddet" class="dt"></div></div>
</main>
<footer class="footer">Weather data from Open-Meteo · AI via Claude API · Built for serious hikers</footer>
...
function wI(c){if(c==null)return"❓";if(c<=1)return"☀️";if(c<=3)return"⛅";if(c<=48)return"🌫️";if(c<=55)return"🌦️";if(c<=65)return"🌧️";if(c<=67)return"🥶";if(c<=77)return"🌨️";if(c<=82)return"🌧️";if(c<=86)return"🌨️";return"⛈️";}
function esc(s){return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");}

function computeScore(d){
  var s=100,iss=[],pos=[];var avg=(d.tmax+d.tmin)/2;
  if(avg>=45&&avg<=70)pos.push("Ideal temperature range");
  else if(d.tmax>90){s-=30;iss.push("Dangerously hot — heat exhaustion risk on steep climbs");}
  else if(d.tmax>85){s-=20;iss.push("Very warm — start early, carry extra water");}
  else if(d.tmax>78){s-=10;iss.push("Warm — pace yourself on uphill sections");}
  else if(d.tmin<20){s-=25;iss.push("Very cold — hypothermia risk, possible ice");}
  else if(d.tmin<32){s-=10;iss.push("Below freezing early — watch for ice on trails");}
  if(d.pprob<10)pos.push("Dry conditions expected");
  else if(d.pprob>70){s-=25;iss.push("High precipitation ("+d.pprob+"%) — wet, slippery trails");}
  else if(d.pprob>40){s-=12;iss.push("Moderate rain chance ("+d.pprob+"%) — bring rain gear");}
  else if(d.pprob>20){s-=5;iss.push("Slight rain chance ("+d.pprob+"%)");}
  if(d.psum>0.5){s-=10;iss.push("Significant accumulation — muddy conditions likely");}
  if(d.wmax<10)pos.push("Calm winds");
  else if(d.gmax>45){s-=30;iss.push("Dangerous gusts to "+Math.round(d.gmax)+" mph — avoid exposed ridges");}
  else if(d.gmax>30){s-=15;iss.push("Strong gusts to "+Math.round(d.gmax)+" mph — caution on ridgelines");}
  else if(d.wmax>20){s-=8;iss.push("Breezy ("+Math.round(d.wmax)+" mph) — may affect balance");}
  if(d.uvmax>8){s-=10;iss.push("Very high UV ("+d.uvmax.toFixed(1)+") — sunburn risk on exposed trails");}
  else if(d.uvmax>5){s-=3;iss.push("Moderate-high UV ("+d.uvmax.toFixed(1)+") — wear sunscreen");}
  else pos.push("Low UV exposure risk");
  var wc=d.wcode||0;
  if(wc>=95){s-=30;iss.push("Thunderstorms — do NOT hike exposed terrain");}
  else if(wc>=66&&wc<=67){s-=20;iss.push("Freezing rain — extremely dangerous");}
  else if(wc>=71&&wc<=77){s-=15;iss.push("Snow expected — traction may be needed");}
...
function bestWin(hrs){
  ...
  var end=Math.min(bs+5,20);function fmt(h){return(h%12||12)+":00 "+(h>=12?"PM":"AM");}return fmt(bs)+" – "+fmt(end);
}
...
function getTips(d){
  ...
  if(d.uvmax>7)t.push("High UV — seek shaded trails or limit ridge exposure to early morning.");
...
function saveKey(){
  ...
  document.getElementById("kst").innerHTML='<span style="color:var(--ac)">✓ Key saved</span>';
  updateKeyBanner();
...
function updateKeyBanner(){
  ...
  if(!apiKey){el.innerHTML='<div style="display:inline-block;background:rgba(92,122,90,.06);border:1px solid rgba(92,122,90,.2);border-radius:10px;padding:10px 18px;font-size:13px;color:var(--txm)">💡 Tap ⚙️ to add your <a href="https://console.anthropic.com/settings/keys" target="_blank">Anthropic API key</a> for AI trail recommendations</div>';}
  else{el.innerHTML='<div style="display:inline-block;background:rgba(92,122,90,.06);border:1px solid rgba(92,122,90,.2);border-radius:10px;padding:10px 18px;font-size:13px;color:var(--ac)">✓ AI Trail Advisor enabled</div>';}
}
...
function drawFavs(){
  ...
  el.innerHTML='<span class="flbl">Favorites:</span>'+favs.map(function(f,i){var act=curLoc&&curLoc.name===f.name&&curLoc.lat===f.lat?" act":"";return'<div class="fc'+act+'" onclick="pickFav('+i+')"><span>📍 '+esc(f.name)+'</span><span class="rx" onclick="event.stopPropagation();rmFav('+i+')">✕</span></div>';}).join("");
}
...
function updSvb(){var b=document.getElementById("svb");if(!curLoc)return;var is=favs.some(function(f){return f.name===curLoc.name&&f.lat===curLoc.lat;});b.className=is?"svb on":"svb";b.textContent=is?"★ Saved":"☆ Save";}
...
function renderDetail(){
  ...
  var stats=[["High / Low",Math.round(day.tmax)+"° / "+Math.round(day.tmin)+"°F"],["Feels Like",Math.round(day.atmax)+"°F"],["Precipitation",day.pprob+"% · "+day.psum+'"'],["Wind",Math.round(day.wmax)+" mph, gusts "+Math.round(day.gmax)],["UV Index",(day.uvmax||0).toFixed(1)],["Sun",day.sr+" – "+day.ss]];

  var winH=day.win?'<div class="wbn"><strong>⏰ Best Hiking Window: </strong>'+day.win+'</div>':"";
  var posH=day.hike.positives.length?'<div style="margin-bottom:8px;display:flex;flex-wrap:wrap;gap:4px">'+day.hike.positives.map(function(p){return'<span class="pt">✓ '+p+'</span>';}).join("")+'</div>':"";
  var issH=day.hike.issues.length?'<div style="margin-bottom:10px">'+day.hike.issues.map(function(s){return'<div class="iss">⚠ '+s+'</div>';}).join("")+'</div>':"";
  ...
  if(cached){
    ...
    aiH='<p style="font-size:12px;color:var(--txmu);margin:6px 0 0">Add your Anthropic API key in ⚙️ Settings to enable AI trail recommendations.</p>';
  }else{
  ...
  var hrows=day.hrs.map(function(hr){
    ...
    var ni=h<6||h>20?" ni":"";
    return'<div class="hrow'+ni+'"><span class="ht2">'+lbl+'</span><span style="font-size:14px">'+wI(hr.wc)+'</span><span>'+Math.round(hr.t)+'°F</span><span style="color:'+(hr.pp>40?"var(--dng)":"var(--txmu)")+'">💧 '+hr.pp+'%</span><span style="color:'+(hr.ws>20?"var(--dng)":"var(--txmu)")+'">💨 '+Math.round(hr.ws)+'</span><span style="color:'+((hr.uv||0)>6?"var(--dng)":"var(--txmu)")+'">'+(hr.uv||0).toFixed(1)+'</span><span style="color:var(--txmu)">'+Math.round(hr.rh)+'%</span></div>';
  }).join("");
  ...
  el.innerHTML=
    ...
    '<div class="icd"><div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:'+(cached?'12':'0')+'px"><h4 style="margin:0">🤖 AI Trail Advisor</h4></div>'+aiH+'</div>'+
...
function renderTrails(trails){
  ...
  return'<div class="icd" style="margin-top:12px;margin-bottom:0;padding:16px"><h4>🥾 Suggested Trails</h4>'+
    trails.map(function(t){
      ...
      var links='<div class="trl-links"><a href="https://www.alltrails.com/search?q='+encodeURIComponent(t.alltrails||t.name)+'" target="_blank" rel="noopener">AllTrails ↗</a><a href="https://www.google.com/search?q='+encodeURIComponent(t.name+" hiking trail")+'" target="_blank" rel="noopener">Google ↗</a>';
      if(t.webUrl)links+='<a href="'+esc(t.webUrl)+'" target="_blank" rel="noopener">Info ↗</a>';
      links+='</div>';
      return'<div class="trl-item"><div class="trl-name">'+esc(t.name)+'</div>'+(stats.length?'<div class="trl-stats">'+esc(stats.join(" · "))+'</div>':'')+(t.why?'<div class="trl-why">'+esc(t.why)+'</div>':'')+links+'</div>';
    }).join("")+'</div>';
}
...
function init(){
  ...
  document.getElementById("pgr").innerHTML=PRESETS.map(function(p,i){
    return'<div class="pcd" onclick="selectLoc(PRESETS['+i+'])"><div class="pn">⛰️ '+esc(p.name)+'</div><div class="pr">'+esc(p.admin1)+', '+esc(p.country)+'</div></div>';
  }).join("");
